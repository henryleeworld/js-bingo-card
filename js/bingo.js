...
function resetSquareColours() {
    for (var i = 0; i < 25; i++) {
        ...
        currentSquare.style.backgroundColor = "#f0f0f5";
    }
    return;
}
...
function checkCornersBingo() {
    ...
    if (sq1.style.backgroundColor == "lightblue" && calledNumbers.includes(parseInt(sq1.value)) &&
		...
		if (!isFinished) {
            document.getElementById("currentCall").innerHTML = "不是有效的賓果！請繼續嘗試！";
            return;
        }
    }
}
...
function youWin(sq1, sq2, sq3, sq4, sq5) {
    ...
    document.getElementById("currentCall").innerHTML = "賓果！";
    alert("賓果！您贏了！");
	isFinished = true;
    // throw new Error("沒錯！剛剛完成遊戲的所有執行！");
}
